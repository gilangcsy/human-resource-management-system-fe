                    ...
                    <div class="card-body">
                        <table class="table table-striped" id="tableWithSearch">
                            ...
                            <tbody>
                                <tr>
                                    <td>1</td>
                                    <td>{{ $claim->approver_one_name }}</td>
                                    <td>
                                        <span class="badge {{ $claim->approval_one_status == 'Approved' ? 'badge-success' : ($claim->approval_one_status == 'Rejected' ? 'badge-danger' : 'badge-warning') }}">{{ $claim->approval_one_status  }}</span>
                                    </td>
                                </tr>
                                @if ($claim->approver_two != null)
                                    <tr>
                                        <td>2</td>
                                        <td>{{ $claim->approver_two_name }}</td>
                                        <td>
                                            <span class="badge {{ $claim->approval_two_status == 'Approved' ? 'badge-success' : ($claim->approval_two_status == 'Rejected' ? 'badge-danger' : 'badge-warning') }}">{{ $claim->approval_two_status  }}</span>
                                        </td>
                                    </tr>
                                @endif
                                @if ($claim->approver_three != null)
                                    <tr>
                                        <td>3</td>
                                        <td>{{ $claim->approver_three_name }}</td>
                                        <td>
                                            <span class="badge {{ $claim->approval_three_status == 'Approved' ? 'badge-success' : ($claim->approval_three_status == 'Rejected' ? 'badge-danger' : 'badge-warning') }}">{{ $claim->approval_three_status }}</span>
                                        </td>
                                    </tr>
                                @endif
                            </tbody>
