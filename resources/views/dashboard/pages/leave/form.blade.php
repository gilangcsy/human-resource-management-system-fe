        ...
        <div class="content ">
            ...
            <div class="container-fluid container-fixed-lg">
                ...
                <div class="card card-default">
                    <div class="card-body">
                        <form method="POST" action="{{ $leave->id == '' ? route('leave.store') : route('leave.update', $leave->id) }}" enctype="multipart/form-data">
                            @csrf
                            @if ($leave->id != '')
                                @method('patch')
                            @endif
                            ...
                            <div class="input-daterange input-group">
                                <input type="date" placeholder="From" class="input-sm form-control" name="start_date" value="{{ $leave->start_date != '' ? \Carbon\carbon::parse(strtotime($leave->start_date))->setTimezone('Asia/Jakarta')->translatedFormat('Y-m-d') : '' }}" required min="2022-01-01" max="2050-01-01" />
                                <div class="input-group-addon">to</div>
                                <input type="date" placeholder="To" class="input-sm form-control" name="end_date" value="{{ $leave->end_date != '' ? \Carbon\carbon::parse(strtotime($leave->end_date))->setTimezone('Asia/Jakarta')->translatedFormat('Y-m-d') : '' }}" required/>
                            </div>

                            <div class="form-group form-group-default mt-3">
                                <label>Description</label>
                                <input type="text" value="{{ old('description', $leave->description) }}" name="description" class="form-control">
                            </div>
